...
int UFOx = 0, UFOy = 15, oldUFOx = 0, oldUFOy = 0; 
void setup() {
  Serial.begin(9600);
...
void loop() {
  ...
  if (t % 10 == 0) {
    ...
    oldUFOx = UFOx;
    oldUFOy = UFOy;
    clrUFO(oldUFOx, oldUFOy);
    if (UFOx < 21 && UFOy > 0){
      (UFOx, UFOy) = moveUFO(UFOx, UFOy);
    }
    else if (UFOx < 21 && UFOy < 15){
      (UFOx, UFOy) = moveUFO(UFOx, UFOy);
    }
...
void clrUFO(int oldUFOx, int oldUFOy){
  int UFOLeftY = oldUFOy - 1, UFOLeftX = oldUFOx,
  UFORightY = oldUFOy + 1, UFORightX = oldUFOx;
  matrix.clrPixel(oldUFOx, oldUFOy), matrix.clrPixel(UFOLeftX, UFOLeftY), matrix.clrPixel(UFORightX, UFORightY);
}
